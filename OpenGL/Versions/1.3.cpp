...
float** addChunk(float chunkX, float chunkZ) {
	...
	for (int x = 0; x < blocksCt; x++) {
		...
		if (!(blocks[x] < 0)) {
			...
			if (yPos[x]+1 > getHeight(BIOME_PLAINS, chunkX * world.chunks.size + xPos[x], chunkZ * world.chunks.size + zPos[x] - 1)) {
				renderBack = true;
				// BACK
			}
			if (yPos[x] + 1 > getHeight(BIOME_PLAINS, chunkX * world.chunks.size + xPos[x], chunkZ * world.chunks.size + zPos[x] + 1)) {
			...
			if (blocks[x - (world.chunks.size + 2)] < 0 && player.position.y < yPos[x]) { // 2 is offset to account for "air" blocks surrounding chunk
				renderBottom = true;
				// BOTTOM
			}

			if (blocks[x + (world.chunks.size + 2)] < && player.position.y > yPos[x]) { // 2 is offset to account for "air" blocks surrounding chunk
				renderTop = true;
				// TOP
			}

			if (yPos[x] + 1 > getHeight(BIOME_PLAINS, chunkX * world.chunks.size + xPos[x] - 1, chunkZ * world.chunks.size + zPos[x])) { // 2 is offset to account for "air" blocks surrounding chunk. 1 accounts for 0
				renderLeft = true;
				// LEFT
			}
