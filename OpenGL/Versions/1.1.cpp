...
float** addChunk(float chunkX, float chunkZ) {
	...
	int *blocks = new int[(world.chunks.size + 2) * 255 * (world.chunks.size + 2)];
	int maxBlocks = 0;
	int totalBlocks = 0;
	...
	float *xPos = new float[1000000];
	float *yPos = new float[1000000];
	float *zPos = new float[1000000];
	int a = 0;
	int iter = 0;
		...
		for (int y = -chunkLimY - 1; y <= chunkLimY + 1; y++) {
			for (int z = -chunkLim - 1; z <= chunkLim + 1; z++) {
				...
				if (x >= -chunkLim &&
					...
					z >= -chunkLim &&
					z <= chunkLim) {
					int heightVal = noiseGenerator.GetNoise(world.chunks.size*chunkX + x, world.chunks.size*chunkZ + z) * 15;

					if (y < heightVal) {
						blocks[totalBlocks] = 9;
						xPos[totalBlocks] = x;
						yPos[totalBlocks] = y;
						zPos[totalBlocks] = z;
					}
	...
	for (int x = 0; x < totalBlocks; x++) {
		//myFile << "type: " << blocks[x] << " ... " << xPos[x] << " " << yPos[x] << " " << zPos[x] << endl;
		if (!(blocks[x] < 0)) {
			...
			if (blocks[x - (world.chunks.size + 2)] < 0) { // 2 is offset to account for "air" blocks surrounding chunk
				renderBottom = true;
				// BOTTOM
			}
			...
			if (blocks[x - (world.chunks.size + 2) * (chunkLimY * 2 + 1 + 2)] <0) { // 2 is offset to account for "air" blocks surrounding chunk. 1 accounts for 0
				renderLeft = true;
				// LEFT
			}
			...
			for (int i = 0; i < 6; i++) {
				...
				for (int j = 0; j < 36; j++) {
					float value;
					if (j % 6 == 0) {
						value = positions[36 * i + j] + xPos[x];
					}
					if (j % 6 == 1) {
						value = positions[36 * i + j] + yPos[x];
					}
					if (j % 6 == 2) {
						value = positions[36 * i + j] + zPos[x];
					}
					if (j % 6 == 3) {
...
void handleInput() {
		...
		if (input.keyboard.isKeyPressed[GLFW_KEY_A]) {
			player.position.x -= player.forwardSpeed * cos(glm::radians(cameraRotY));
			player.position.z -= player.forwardSpeed * sin(glm::radians(cameraRotY));
		}
...
void renderChunk(float**& chunk) {
	glBufferData(GL_ARRAY_BUFFER, 1000000, chunk[2], GL_DYNAMIC_DRAW);
...
void generateChunks() {
		...
		for (int x = 0; x < chunksToGenerateAcross; x++) {
			for (int z = 0; z < chunksToGenerateAcross; z++) {
				bool alreadyExists = false;
				int a = 0;
				for (int i = 0; i < world.chunks.totalCount; i++) {
					if (*chunks[i] != 0) {
						if (*chunks[i][0] == world.chunks.current.x + x - world.chunks.toGenerate &&
				...
				if (!alreadyExists) {
					...
					chunks[world.chunks.totalCount] = addChunk(world.chunks.current.x + x - world.chunks.toGenerate, world.chunks.current.z + z - world.chunks.toGenerate);
...
int main(void)
{
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/*
		if (player.position.y > getHeight(BIOME_PLAINS, player.position.x, player.position.z)) {
			if (!player.isJumping) {
				
			}
			else{
				player.position.y = getHeight(BIOME_PLAINS, player.position.x, player.position.z) + 1.5 + sin(counter);
			}
		}
		*/
